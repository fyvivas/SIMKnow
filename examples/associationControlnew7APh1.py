...
def topology(isVirtual, acceso):
	...
	if asoc == "ssf":
		f_ssf= open("/home/wifi/new/mininet-wifi/scripts/output/ssf.txt","w+")
		f_ssf.close()
	elif asoc == "llf":
		f_llf= open("/home/wifi/new/mininet-wifi/scripts/output/llf.txt","w+")
		f_llf.close()
	elif asoc == "slf":
		f_slf= open("/home/wifi/new/mininet-wifi/scripts/output/slf.txt","w+")
		f_slf.close()
	elif asoc == "topsis":
		f_topsis= open("/home/wifi/new/mininet-wifi/scripts/output/topsis.txt","w+")
		f_topsis.close()
	elif asoc == "saw":
		f_topsis= open("/home/wifi/new/mininet-wifi/scripts/output/saw.txt","w+")
		f_topsis.close()
	elif asoc == "rf":
		f_rf= open("/home/wifi/new/mininet-wifi/scripts/output/rf.txt","w+")
		f_rf.close()
	elif asoc == "dl":
		f_rf= open("/home/wifi/new/mininet-wifi/scripts/output/dl.txt","w+")
		f_rf.close()
    ...
    sta1 = net.addStation('sta1', mac='00:00:00:00:00:02', ip='10.0.0.2/8', range = 20, bw_d = 2000, battery = 100)
...
#    sta1 = net.addStation('sta1', ip='10.0.1.123', mac='00:00:00:02:09:01', position='40.48,100.36,0.00', range = 20, speed=1, bw_d = 2000, battery = 100, id_group = "", hist = [[0, 0, 0],[0, 0, 0],["","",""],["","","","","","","","","","","","","",""]])
    ...
    #sta2 = net.addStation('sta2', mac='00:00:00:00:00:03', ip='10.0.0.4/8', min_x=10, max_x=90, min_y=30, max_y=35, min_v=1, max_v=2, min_wt=0, max_wt=10, battery = 100, initPos='110.0,35.0,0')
    ...
    ap1 = net.addAccessPoint('ap1', ssid='ssid-ap1', mode='n', channel='1',
                             position='50,50,0', antennaGain = 5, range=50, battery = 100, maxDis = 15, bw_ap = 15000)
...
#ap1 = net.addAccessPoint('ap1', ssid='ap1-ssid', mode='n', channel='1', position='50.0,150.0,0', antennaGain = 5, range = 45, battery = 100, maxDis = 15, bw_ap = 15000) # gain 8 = (txpow = 3 + gain = 5)
    ...
    ap4 = net.addAccessPoint('ap4', ssid='ssid-ap4', mode='n', channel='1',
                             position='120,50,0', antennaGain = 5, range=50, battery = 100, maxDis = 15, bw_ap = 15000)
    
    ap5 = net.addAccessPoint('ap5', ssid='ssid-ap5', mode='n', channel='1',
                             position='100,120,0', antennaGain = 5, range=50, battery = 100, maxDis = 15, bw_ap = 15000)
    ap6 = net.addAccessPoint('ap6', ssid='ssid-ap6', mode='n', channel='6',
                             position='120,150,0', antennaGain = 3, range=30, battery = 100, maxDis = 15, bw_ap = 15000)
    #if not isVirtual:
    print ("acceso association control %s" %acceso)
    ap2 = net.addAccessPoint('ap2', ssid='ssid-ap2', mode='n', channel='6',
                             position='85,80,0', antennaGain = 3, range=30, battery = 100, maxDis = 15, bw_ap = 15000)
    ap3 = net.addAccessPoint('ap3', ssid='ssid-ap3', mode='n', channel='8',
                             position='85,30,0', antennaGain = 3, range=30, battery = 100, maxDis = 15, bw_ap = 15000)
    ap7 = net.addAccessPoint('ap7', ssid='ssid-ap7', mode='n', channel='8',
                             position='120,100,0', antennaGain = 3, range=30, battery = 100, maxDis = 15, bw_ap = 15000)
...
# METODO DE MOVIMIENTO 1 - para mover un dispositivo de un punto a otro con el metodo net.startMobility
    ...
    net.startMobility( time=1, repetitions=1, ac_method='topsis')
    net.mobility( sta1, 'start', time=1, position= u1_start)
    net.mobility( sta1, 'stop', time=31, position= u1_stop)
...
#os.system('ip link set hwsim0 up')
    ...
    #makeTerm( sta2, cmd="bash -c 'iperf -s -i 1;'" )
    sleep(1)
    #makeTerm( sta1, cmd="bash -c 'iperf -c 10.0.0.4 -t 30;'" )

    makeTerm( sta1, cmd="bash -c 'iperf -c 10.0.0.4 -u -b 2M -t 40 -p 5566;'" )

#ITGSend -T TCP  -a 10.0.0.4 -C 10 -c 100 -t 60000 -l sender21.log -x receiver12.log

    #-makeTerm( sta2, cmd="bash -c 'ITGRecv -l receiverm3.log;'" )
    #-sleep(1)
    ...
    #-makeTerm( sta1, cmd="bash -c 'ITGSend script_file1 -l senderm3.log;'" )
