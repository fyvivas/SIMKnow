...
#ap1 = net.addAccessPoint('ap1', ssid='ap1-ssid', mode='n', channel='1', position='50.0,150.0,0', antennaGain = 5, range = 45, battery = 100, maxDis = 15, bw_ap = 15000)


    ap1 = net.addAccessPoint('ap1', ssid='ssid-ap1', mode='n', channel='1', antennaGain = 6,
                             position='50,50,0', range=50, battery = 100, maxDis = 35)
    ap4 = net.addAccessPoint('ap4', ssid='ssid-ap4', mode='n', channel='11', antennaGain = 6,
                             position='120,50,0', range=50, battery = 100, maxDis = 35)
    ap5 = net.addAccessPoint('ap5', ssid='ssid-ap5', mode='n', channel='1', antennaGain = 6,
                             position='100,120,0', range=50, battery = 100, maxDis = 35)

    ap6 = net.addAccessPoint('ap6', ssid='ssid-ap6', mode='n', channel='6', antennaGain = 4,
                             position='120,150,0', range=30, battery = 100, maxDis = 15)
    ap2 = net.addAccessPoint('ap2', ssid='ssid-ap2', mode='n', channel='6', antennaGain = 4,
                             position='85,80,0', range=30, battery = 100, maxDis = 15)
    ap3 = net.addAccessPoint('ap3', ssid='ssid-ap3', mode='n', channel='8', antennaGain = 4,
                             position='85,30,0', range=30, battery = 100, maxDis = 15)
    ap7 = net.addAccessPoint('ap7', ssid='ssid-ap7', mode='n', channel='11', antennaGain = 4,
                             position='120,100,0', range=30, battery = 100, maxDis = 15)
    ...
    if '-r' in args:
        ...
        print ("acceso association control ssf")
    if '-t' in args:
        net.setAssociationCtrl( 'topsis' )
        print ("acceso association control topsis")
    if '-k' in args:
        net.setAssociationCtrl( 'simknow' )
        print ("acceso association control simknow")
...
# METODO DE MOVIMIENTO 1 - para mover un dispositivo de un punto a otro con el metodo net.startMobility
    ...
    if '-k' in args:
        net.startMobility( time=5, repetitions=1, ac_method='simknow')
	print ("startMobility control simknow")
    
    net.mobility( sta1, 'start', time=5, position= u1_start)
    
    if '-s' in args:
        print ("User slow Mobility")
        net.mobility( sta1, 'stop', time=165, position= u1_stop) #135
        net.stopMobility( time=175 )
    if '-m' in args:
        print ("User Medium Mobility")
        net.mobility( sta1, 'stop', time=65, position= u1_stop)
        net.stopMobility( time=75 )
    if '-h' in args:
        print ("User High Mobility")
        net.mobility( sta1, 'stop', time=21, position= u1_stop)
	...
	f.write('%s\tSTA\tOldAP\tNewAP\tAPInRange\tSpeed\t%s\n' %(str(tiempoInit), str(u1_start)))
	f.close()
    if '-t' in args:
    ...
    """
    ap1.setTxPower(4, intf='ap1-wlan1')
    ap4.setTxPower(4, intf='ap4-wlan1')
    ap5.setTxPower(4, intf='ap5-wlan1')    

    ap2.setTxPower(2, intf='ap2-wlan1')
    ap3.setTxPower(2, intf='ap3-wlan1')
    ap6.setTxPower(2, intf='ap6-wlan1')
    ap7.setTxPower(2, intf='ap7-wlan1')
    """
...
#os.system('ip link set hwsim0 up')
    ...
    if '-s' in args:
		if '-d' in args:
			makeTerm( sta2, cmd="bash -c 'ITGRecv -l recvslow.log;'" )
    			sleep(1)
			makeTerm( sta1, cmd="bash -c 'ITGSend script_file_slow -l sendslow.log;'" )
		if '-i' in args:
		    	makeTerm( sta2, cmd="bash -c 'iperf -s -u -p 5566 -i 1 > riperfslow;'" )
		    	sleep(1)
		    	makeTerm( sta1, cmd="bash -c 'iperf -c 10.0.0.4 -u -b 2M -t 180 -p 5566;'" )
    if '-m' in args:
		if '-d' in args:
			makeTerm( sta2, cmd="bash -c 'ITGRecv -l recvmed.log;'" )
		    	sleep(1)
			makeTerm( sta1, cmd="bash -c 'ITGSend script_file_med -l sendmed.log;'" )
		if '-i' in args:
		    	makeTerm( sta2, cmd="bash -c 'iperf -s -u -p 5566 -i 1 > riperfmed;'" )
    			sleep(1)
    			makeTerm( sta1, cmd="bash -c 'iperf -c 10.0.0.4 -u -b 2M -t 80 -p 5566;'" )
    if '-h' in args:
		if '-d' in args:
			makeTerm( sta2, cmd="bash -c 'ITGRecv -l recvhigh.log;'" )
    			sleep(1)
			makeTerm( sta1, cmd="bash -c 'ITGSend script_file_high -l sendhigh.log;'" )
		if '-i' in args:
		    	makeTerm( sta2, cmd="bash -c 'iperf -s -u -p 5566 -i 1 > riperfhigh;'" )
    			sleep(1)
    			makeTerm( sta1, cmd="bash -c 'iperf -c 10.0.0.4 -u -b 2M -t 30 -p 5566;'" )
    ...
    #makeTerm( sta1, cmd="bash -c 'ITGSend -T TCP  -a 10.0.0.4 -C 10 -c 100 -t 80000 -l sender21.log -x receiver12.log;'" )
    #makeTerm( sta1, cmd="bash -c 'ITGSend script_file_slow -l senderm1.log;'" )
