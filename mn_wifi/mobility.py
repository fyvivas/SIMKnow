...
#-----------------------
    ...
    def check_association(cls, sta, wlan, ap_wlan):
        ...
        cls.ap_in_range(sta, ap, wlan, dist)
        """
        if ((str(cls.ac) == 'topsis') or (str(cls.ac) == 'simknow')):
	    #print ("check_association")
	    #print('{"check_association":"%s"}\n' %(str(cls.ac)))
            ...
            if len(aps) > 1:
                ...
                value = associationControl(sta, apsc, wlan, cls.ac)
		...
		changeAP = value.changeAP
                ...
                Association.associate_infra(sta, apsc, wlan=wlan, ap_wlan=ap_wlan)
                        ...
                        for ap in cls.aps:
                            for ap_wlan in range(len(ap.params['wlan'])):
                        		...
                        		and ('encrypt' in node.params and 'wpa' in node.params['encrypt'][wlan])):
                        	    	    for ap in cls.aps:
                        	                if node.params['associatedTo'][wlan] == '':
                        	            	    Association.associate_infra(node, ap, wlan=wlan, ap_wlan=0)
                        	            	    node.params['associatedTo'][wlan] = 'active_scan'
                        	            	    if Association.bgscan:
                        	                	node.params['associatedTo'][wlan] = 'bgscan'
                        		else:
